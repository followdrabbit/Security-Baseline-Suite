...
const BaselineMindMap: React.FC<Props> = ({ technologyName, controls, categoryLabels }) => {
  ...
  const exportToPng = useCallback(() => {
    ...
    // Compute styles and inline them for proper rendering
    ...
    // Set background on exported SVG
    ...
    // Inline text fills that use CSS variables
    clone.querySelectorAll('text').forEach(text => {
      const fill = text.getAttribute('fill') || '';
      if (fill.includes('var(--muted-foreground)')) {
        text.setAttribute('fill', mutedFg ? `hsl(${mutedFg})` : '#888');
      } else if (fill.includes('var(--primary)')) {
        text.setAttribute('fill', primaryColor ? `hsl(${primaryColor})` : '#c8a84e');
      }
    });
